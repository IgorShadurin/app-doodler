...
test("buildGenerationPlan creates language x preset combinations", () => {
  const plan = buildGenerationPlan(["iphone-6-7", "ipad-12-9"], ["en", "fr"]);

  assert.equal(plan.length, 4);
  assert.deepEqual(plan[0], { presetId: "iphone-6-7", languageCode: "en" });
...
test("buildGenerationPlan returns empty when any side is empty", () => {
  ...
  assert.equal(buildGenerationPlan(["iphone-6-7"], []).length, 0);
});
